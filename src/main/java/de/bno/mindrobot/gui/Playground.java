...
public class Playground extends JComponent implements RobotControl, SignalListener
{
	...
	private static final Dimension MINIMAL_FELD_SIZE = new Dimension(Pixel.pointsToPixel(50), Pixel.pointsToPixel(50));

	private static final Dimension NORMAL_FELD_SIZE = new Dimension(Pixel.pointsToPixel(100), Pixel.pointsToPixel(100));
	...
	private void updateControllerSize()
	{
		...
		Dimension size = new Dimension(Math.min(prefSize.width, getWidth()), Math.min(prefSize.height, getHeight()));
	...
	protected void paintComponent(Graphics g)
	{
		...
		{
			img = gfxConf.createCompatibleImage(getWidth(), getHeight(), Transparency.TRANSLUCENT);
		}
	...
	private void paintAvatar(Graphics g)
	{
		int size = (int) Math.floor(
			Math.min(getWidth() / ((double) spielfeld.getWidth()), getHeight() / ((double) spielfeld.getHeight())));
		...
		for (int y = 0; y < spielfeld.getHeight(); y++)
		{
			for (int x = 0; x < spielfeld.getWidth(); x++)
			{
				...
				{
					drawTile(g, size, inset, offsetWidth, offsetHeight, getAvatar(directionAvatar), y, x);
					if (speech != null && !speech.isEmpty())
					{
						...
						g.fill3DRect(xt, yt, fmt.stringWidth(speech) + 5, fmt.getHeight() + 5, true);
	...
	private void paintFloor(Graphics g)
	{
		...
		for (int y = 0; y < spielfeld.getHeight(); y++)
		{
			for (int x = 0; x < spielfeld.getWidth(); x++)
			{
				tile = getTile(x, y);
				g.setColor(Color.BLACK);
				g.fillRect(offsetWidth + x * size, offsetHeight + y * size, size, size);
	...
	private void drawTile(Graphics g, int size, int inset, int offsetWidth, int offsetHeight, Image tile, int y, int x)
	{
		g.drawImage(tile, getPosForPainting(size, inset, offsetWidth, x),
			getPosForPainting(size, inset, offsetHeight, y), offsetWidth + x * size + size - inset,
			offsetHeight + y * size + size - inset, 0, 0, tile.getWidth(this), tile.getHeight(this), this);
	}
	...
	public void moveAvatarToLocation(Location l)
	{
		...
		if (l.getX() >= 0 && l.getY() >= 0 && l.getX() < spielfeld.getWidth() && l.getY() < spielfeld.getHeight())
		{
	...
	private boolean moveForwards_()
	{
		...
		Location newLocation = Avatar.fieldInFront(actualLocation, getAvatarsDirection());
	...
	private boolean moveBackwards_()
	{
		...
		Location actualLocation = getAvatarPosition();
		Location newLocation = Avatar.fieldBehind(actualLocation, getAvatarsDirection());
	...
	public boolean isBlockedFieldInFront()
	{
		...
		Feld fieldInFront = spielfeld.getFeld(newLocation.getX(), newLocation.getY());
	...
	public boolean standOnConfusingField()
	{
		...
		Feld field = spielfeld.getFeld(actualLocation.getX(), actualLocation.getY());
	...
	{
		int lastGoal = spielfeld.getLastGoal();
		return lastGoal;
	}
